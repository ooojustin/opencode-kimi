...
6. **The model list comes from `/coding/v1/models`, not from user config.** opencode model keys ARE wire ids, so there is no alias and nothing rewrites the body `model` field. The plugin calls `GET /coding/v1/models` at login, on first loader use, and on every token refresh (mirroring kimi-cli's `refresh_managed_models` in `research/kimi-cli/src/kimi_cli/auth/platforms.py`), and `provider.models` surfaces every returned entry as its own selectable model, backfilling `limit.context`, display name, and media capabilities. Config-declared entries are enriched, never replaced, so an explicit `name` still wins and the provider stays usable from config alone when discovery fails. Do not strip the `kimi-` prefix; send whatever the server returned. Discovery failures are non-fatal (warm cache still works; 401 retry flushes broken tokens).
